...
//    v.3.61  - UnitTest setup
//----------------------------------------------------------------------------- 

#ifndef FRAMEWORK_H
#define FRAMEWORK_H

#define FRAMEWORK_VER "3.61"
...
#define UNIT_TEST_CPP_H
		...
		#define UNIT_TEST_MEMORYCHECK_BEGIN int UnitTest_MemLeakCount_Start = 0;
...
#ifdef _DEBUG
		#define UnitTest_SetName(A,B)		  Debug::SetName(A,B); \
											  UnitTest_MemLeakCount_Start += 1;
#else
		#define UnitTest_SetName(A,B)		  Debug::SetName(A,B);
#endif
		...
		public:
			...
			static bool GetCRC(const char *const pFileName, uint32_t &crc)
			{
				...
				else
				{
					...
					{
						pCRC->privFile(buf, fread(buf, 1, sizeof(buf), pFile), crc);
					}
